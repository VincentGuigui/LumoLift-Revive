...
/** Port of the app's AndroidBleConnection, trimmed to what this probe needs. */
@SuppressLint("MissingPermission")
internal class BleConnection(private val context: Context) {
    ...
    private suspend fun scan(): Pair<BluetoothDevice, String> = withTimeout(20_000) {
        val result = CompletableDeferred<Pair<BluetoothDevice, String>>()
        ...
        val callback = object : ScanCallback() {
            override fun onScanResult(callbackType: Int, scanResult: ScanResult) {
                ...
                if (name.startsWith("Lumo", ignoreCase = true) || services.contains(ParcelUuid(PRIMARY_SERVICE))) {
                    if (result.complete(scanResult.device to name)) scanner.stopScan(this)
                }
            }

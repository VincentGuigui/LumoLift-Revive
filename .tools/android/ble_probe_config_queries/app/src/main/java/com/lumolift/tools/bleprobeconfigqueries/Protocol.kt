...
/** Port of python/lumolift/protocol.py, limited to what this probe needs. */
object Protocol {
    ...
    /** Decode one null-terminated JSON response payload, as produced by the sensor. */
    fun decodeJsonPayload(payload: ByteArray): JSONObject {
        val terminator = payload.indexOf(0)
        require(terminator >= 0) { "JSON response is not null-terminated" }
        require(payload.drop(terminator + 1).all { it == 0.toByte() }) {
            "JSON response has non-zero data after its terminator"
        }
        val text = payload.copyOfRange(0, terminator).toString(Charsets.UTF_8)
